...
//	if(wxGetApp().PickPosition(_T("Pick finish position"), pos)){
...
void CProgramCanvas::AppendText(double value)
{
	...
	ss << value;
	wxGetApp().m_program_canvas->m_textCtrl->AppendText(ss.str().c_str());
}

void CProgramCanvas::AppendText(int value)
{
	wxGetApp().m_program_canvas->m_textCtrl->AppendText(wxString::Format(_T("%d"), value));
}
