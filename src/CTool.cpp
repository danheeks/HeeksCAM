		...
		default:
			{
				...
				if(toolIcon == NULL)toolIcon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/tool.png")));
				return *toolIcon;
			}
	}
}
...
void CTool::glCommands(bool select, bool marked, bool no_color)
{
	...
	if(wxGetApp().m_marked_list->ObjectMarked(this) && !no_color)
	{
		if(!m_pToolSolid)
		{
			try {
				TopoDS_Shape tool_shape = GetShape();
				m_pToolSolid = new CSolid(*((TopoDS_Solid *)&tool_shape), NULL, HeeksColor(234, 123, 89), 1.0f);
			} // End try
...
{
	CToolDlg dlg(wxGetApp().m_frame, (CTool*)object);
...
{
	return wxGetApp().m_program->Tools();
}
