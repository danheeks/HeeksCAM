...
const wxBitmap &CDrilling::GetIcon()
{
	...
	if(icon == NULL)icon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/drilling.png")));
	return *icon;
}
...
Python CDrilling::AppendTextToProgram()
{
	...
	{
		HeeksObj* object = wxGetApp().GetIDObject(PointType, *It);
...
void CDrilling::glCommands(bool select, bool marked, bool no_color)
{
	...
	if (select)
	{
		for (std::list<int>::iterator It = m_points.begin(); It != m_points.end(); It++)
		{
			HeeksObj* point = wxGetApp().GetIDObject(PointType, *It);
			if (point)point->glCommands(select, marked, no_color);;
		}
	}

	else if (wxGetApp().m_marked_list->ObjectMarked(this))
	{
		wxGetApp().background_color[0].best_black_or_white().glColor();
...
void CDrilling::WriteXML(TiXmlNode *root)
{
	...
	for (std::list<int>::iterator It = m_points.begin(); It != m_points.end(); It++)
	{
		...
		wxGetApp().LinkXMLEndChild( element, point );
...
HeeksObj* CDrilling::ReadFromXMLElement(TiXmlElement* element)
{
	...
	for (TiXmlElement* pElem = TiXmlHandle(element).FirstChildElement().Element(); pElem; pElem = pElem->NextSiblingElement())
	{
		...
		if(name == "params"){
			...
			for(TiXmlElement* child = TiXmlHandle(element).FirstChildElement().Element() ; child; child = child->NextSiblingElement())
			{
				int type, id;
