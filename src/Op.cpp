...
const wxBitmap& COp::GetInactiveIcon()
{
	...
	if(icon == NULL)icon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/noentry.png")));
	return *icon;
}
...
void COp::GetProperties(std::list<Property *> *list)
{
	...
	if(UsesTool()){
		...
		HTypeObjectDropDown::GetObjectArrayString(ToolType, wxGetApp().m_program->Tools(), tools_for_GetProperties);
		int choice = 0;
...
Python COp::AppendTextToProgram()
{
	...
	if(UsesTool())
	{
		python << wxGetApp().SetTool(m_tool_number); // Select the correct  tool.
	}
...
{
	return wxGetApp().m_program->Operations();
}
