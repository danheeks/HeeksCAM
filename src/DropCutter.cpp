...
double DropCutter::VertexTest(const Cutter &c, const double *e, const double *p)
{
	...
	if (q > c.R + wxGetApp().m_geom_tol)
	{
		// vertex is outside cutter. no need to do anything!
		return -10000000.0;
	}
	else if (q <= (c.R - c.r) + wxGetApp().m_geom_tol)
	{
...
double DropCutter::FacetTest(const Cutter &cu, const double *e, const GTri &t)
{
	...
	if ((fabs(a) < wxGetApp().m_geom_tol) && (fabs(b) < wxGetApp().m_geom_tol))
	{
...
static bool isinrange(double start, double end, double x)
{
	...
	if ((x >= start - wxGetApp().m_geom_tol) && (x <= end + wxGetApp().m_geom_tol))
		return true;
	else
		return false;
}
...
double DropCutter::EdgeTest(const Cutter &cu, const double *e, const double *p1, const double *p2)
{
	...
	if (fabs(cu.r) < wxGetApp().m_geom_tol)
	{
		...
		else // we are inside the cutter
		{
			...
			if ((start[0] > xc + wxGetApp().m_geom_tol) && (xc + wxGetApp().m_geom_tol< end[0]))
				return -10000000.0;
			else if ((end[0] < xc - wxGetApp().m_geom_tol) && (xc + wxGetApp().m_geom_tol > start[0]))
				return -10000000.0;
			else
				return zc;
	...
	else// if (cu.r > 0)
	{
		...
		if (l > cu.R + wxGetApp().m_geom_tol) // edge is outside of the cutter
			return -10000000.0;
		else if (((cu.R-cu.r)<l - wxGetApp().m_geom_tol)&&(l<=cu.R + wxGetApp().m_geom_tol))
		{    // toroidal case
		...
		if (fabs(start[2] - end[2]) < wxGetApp().m_geom_tol)
		{
			if ((cu.R-cu.r)<l - wxGetApp().m_geom_tol)
			{
				// half-ellipse case
				xc=0;
		...
		if (((cu.R - cu.r) < l - wxGetApp().m_geom_tol) && (cu.R <= l + wxGetApp().m_geom_tol))
		{
			// half-ellipse case
