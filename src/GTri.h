...
public:
	...
	static bool box_in_box(double *this_box, double *box){
		if(this_box[0]<box[0]-wxGetApp().m_geom_tol){
			// left of tri is left of box
			if(this_box[2]<box[0]-wxGetApp().m_geom_tol){
				// right of tri is left of box
				return false;
			}
			else if(this_box[2]<box[2] + wxGetApp().m_geom_tol){
				// right of tri is in box
				if(this_box[1]<box[1]-wxGetApp().m_geom_tol){
					// bottom of tri is below box
					if(this_box[3]<box[1]-wxGetApp().m_geom_tol){
				...
				else if(this_box[1]<box[3]+wxGetApp().m_geom_tol){
				...
				if(this_box[1]>box[1]-wxGetApp().m_geom_tol && this_box[3]<box[3]+wxGetApp().m_geom_tol){
		...
		else if(this_box[0]<box[2]+wxGetApp().m_geom_tol){
