...
const wxBitmap &CPattern::GetIcon()
{
	...
	if(icon == NULL)icon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/pattern.png")));
	return *icon;
}
...
{
	PatternDlg dlg(wxGetApp().m_frame, (CPattern*)object);
...
{
	return wxGetApp().m_program->Patterns();
}
