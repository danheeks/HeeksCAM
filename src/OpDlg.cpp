...
{
	if(want_tool_control)leftControls.push_back(MakeLabelAndControl(_("Tool"), m_cmbTool = new HTypeObjectDropDown(this, ID_TOOL, ToolType, wxGetApp().m_program->Tools())));
	else m_cmbTool = NULL;
	leftControls.push_back(MakeLabelAndControl(_("Pattern"), m_cmbPattern = new HTypeObjectDropDown(this, ID_PATTERN, PatternType, wxGetApp().m_program->Patterns())));
	leftControls.push_back(MakeLabelAndControl(_("Surface"), m_cmbSurface = new HTypeObjectDropDown(this, ID_SURFACE, SurfaceType, wxGetApp().m_program->Surfaces())));
...
void OpDlg::SetPictureByWindow(wxWindow* w)
{
	if(w == m_cmbPattern)
	{
		...
		if(pattern_id != 0)
		{
			PatternDlg::RedrawBitmap(m_picture->m_bitmap, (CPattern*)(wxGetApp().GetIDObject(PatternType, pattern_id)));
