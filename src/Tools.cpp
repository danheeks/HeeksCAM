...
const wxBitmap &CTools::GetIcon()
{
	...
	if(icon == NULL)icon = new wxBitmap(wxImage(wxGetApp().GetResFolder() + _T("/icons/tools.png")));
	return *icon;
}
...
class ExportTools: public Tool{
	...
	void Run()
	{
		...
		if(m_for_default)
		{
			previous_path = wxGetApp().GetResourceFilename(wxT("default.tooltable"), true);
		}
		else
		{
			// Prompt the user to select a file to import.
			wxFileDialog fd(wxGetApp().m_frame, _T("Select a file to export to"),
		...
		for (HeeksObj *tool = wxGetApp().m_program->Tools()->GetFirstChild();
			tool != NULL;
			tool = wxGetApp().m_program->Tools()->GetNextChild() )
		{
			tools.push_back( tool );
		} // End for
		wprintf(wxT("Exporting tools as ") + previous_path  + wxT("\n"));
		wxGetApp().SaveXMLFile( tools, previous_path.c_str(), false );
	}
...
void ImportToolsFile( const wxChar *file_path )
{
    ...
    {
        wxGetApp().Remove( *l_itObject );
    } // End for

    // And read the default speed references.
    // wxGetApp().OpenXMLFile( _T("default.speeds"), true, wxGetApp().m_program->m_tools );
    wxGetApp().OpenXMLFile( file_path, wxGetApp().m_program->Tools() );
}
...
class ImportTools: public Tool{
	...
	void Run()
	{
		...
		if(m_for_default)
		{
			previous_path = wxGetApp().GetResourceFilename(wxT("default.tooltable"));
		}
		else
		{
			// Prompt the user to select a file to import.
			wxFileDialog fd(wxGetApp().m_frame, _T("Select a file to import"),
				standard_paths.GetUserConfigDir().c_str(), previous_path.c_str(),
				wxString(_("Known Files")) + _T(" |*.heeks;*.HEEKS;")
				+ _T("*.tool;*.TOOL;*.Tool;")
				+ _T("*.tools;*.TOOLS;*.Tools;")
...
void CTools::GetTools(std::list<Tool*>* t_list, const wxPoint* p)
{
	...
	HeeksCADapp::GetNewToolTools(t_list);
